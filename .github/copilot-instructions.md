...
Run the `FetchDataFunctions` crawler app locally with `dotnet run` from `FetchDataFunctions/` (the recommended path for .NET isolated — `func start` warns and may fail to load worker extensions). It needs **Azurite** running for `AzureWebJobsStorage` (blob secret repo + timer/durable listeners **and the durable orchestration state**); without it the host cancels startup. The durable backend is **Azure Storage**, so no separate emulator is needed — durable state goes to Azurite alongside `AzureWebJobsStorage` (`TASKHUB_NAME=default` locally). Timer CRON schedules come from app settings (`HutsUpdateSchedule`, `AvailabilityUpdateSchedule`, `HuettenHolidayAvailabilityUpdateSchedule`, `CleanupSchedule`). Timer crawlers early-return in Development, but you can drive a single hut on demand: `GET http://localhost:7071/api/UpdateAvailabilityHttpTriggered?hutid=<id>` (also `UpdateHutHttpTriggered`), which invoke the same activity code (incl. the EF-Core SQL writes) directly. Both Functions apps' `local.settings.json` point `DatabaseConnectionString` at the **real prod Azure SQL** DB, so it's queryable directly for diagnosis.
...
- Durable orchestrations use the **Azure Storage** backend (the default provider; the only BYO durable backend supported on Flex Consumption). `host.json` sets only `hubName` from `%TASKHUB_NAME%`; the provider reuses the identity-based `AzureWebJobsStorage` connection. They process huts **sequentially, one at a time, with a 1-second delay between huts** via the shared `Functions/DurableSequentialProcessor.cs` helper (each provider now returns a hut's availability in a single request, so the old batching is unnecessary); use `context.CreateReplaySafeLogger` inside orchestrators. The per-day availability upsert/diff logic is the pure, unit-tested `AvailabilityReconciler`.
