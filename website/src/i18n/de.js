export default {
   message: {
      siteTitle: "AV-Hüttenbuchung",
      map: "Karte",
      list: "Liste",
      info: "Info",
      search: "Suche",
      language: "Sprache",
      theme: "Design",
      theme_auto: "System",
      theme_light: "Hell",
      theme_dark: "Dunkel",
      loading: "Lade...",
      footerText: "Dies ist ein privates Projekt ohne Verbindung zu den Alpenvereinen",
      onlineBooking: "Online Buchung",
      onlineBookingInactive: "Online Buchung nicht aktiv",
      lastUpdated: "Letztes Update",
      hutfullNotify: "Benachrichtige mich, wenn ein Platz frei wird",
      showHutDetails: "Zeige Hütten Details",
      showOnMap: "Zeige auf Karte",
      hut: "Hütte",
      country: "Land",
      region: "Region",
      coordinates: "Koordinaten",
      hutAdded: "Hütte hinzugefügt",
      date: "Datum",
      beds: "Betten",
      typeOfAccommodation: "Zimmerkategorie",
      localeen: "English",
      localede: "Deutsch",
      hutClosed: "Hütte geschlossen",
      hutNotYetActive: "Hütte noch nicht aktiviert",
      hutWebsite: "Hütten Webseite",
      submit: "Absenden",
      formSuccessfullySubmitted: "Erfolgreich abgesendet",
      admin: "Admin",
      editHut: "Hütte bearbeiten",
      delete: "Löschen",
      edit: "Bearbeiten",
      save: "Speichern",
      cancel: "Abbrechen",
      close: "Schließen",
      confirmDelete: "Löschen bestätigen",
      confirmDeleteText: "Sind Sie sicher, dass Sie die Hütte löschen möchten",
      yes: "Ja",
      no: "Nein",
      enabled: "Aktiviert",
      isDeleted: "Gelöscht",
      website: "Webseite",
      onlineBookingLink: "Online-Buchungslink",
      latitude: "Breitengrad",
      longitude: "Längengrad",
      altitude: "Höhe",
      actions: "Aktionen",
      name: "Name",
      login: "Anmelden (admin)",
      logout: "Abmelden",
      manuallyEdited: "Manuell bearbeitet",
      addLocation: "Standort hinzufügen",
      filterByWeekdays: "Filtern nach Wochentagen",
      searchHuts: "Hütten suchen...",
      noResultsFound: "Keine Verfügbarkeit für ausgewählte Wochentage gefunden",
      clearFilter: "Filter zurücksetzen",
      noAvailabilityInfo: "Keine Belegungsinfo verfügbar. Bitte direkt auf der Buchungsseite nachsehen.",
      allDatesClosedTitle: "Alle Termine sind derzeit geschlossen",
      sourceAlpenvereine: "Alpenvereine",
      sourceHuettenHoliday: "Hütten-Holiday",
      source: "Quelle",
      sunday: "Sonntag",
      monday: "Montag",
      tuesday: "Dienstag",
      wednesday: "Mittwoch",
      thursday: "Donnerstag",
      friday: "Freitag",
      saturday: "Samstag",
      january: "Januar",
      february: "Februar",
      march: "März",
      april: "April",
      may: "Mai",
      june: "Juni",
      july: "Juli",
      august: "August",
      september: "September",
      october: "Oktober",
      november: "November",
      december: "Dezember",
      mon: "Mo",
      tue: "Di",
      wed: "Mi",
      thu: "Do",
      fri: "Fr",
      sat: "Sa",
      sun: "So",
   },
   mapPage: {
      availabilityAtDate: "Verfügbarkeit am",
      numberOfBeds: "Anzahl Betten",
      bedCategory: "Zimmerkategorie",
      freeBeds: "Freie Betten",
      noAvailabilityInfo: "Keine Belegungsinfo verfügbar. Bitte direkt auf der Buchungsseite nachsehen.",
      anyBeds: "egal",
      zoomIn: "Zoom",
      filterTitle: "Filter",
   },
   hutListPage: {
      allHuts: "Alle Hütten",
      hutCount: "{count} Hütten",
   },
   infoPage: {
      title: "Alpine Hütten Projekt",
      infoHeader: "Willkommen",
      aboutTitle: "Über dieses Projekt",
      howItWorksTitle: "Funktionsweise",
      openSourceTitle: "Open Source",
      viewOnGithub: "Auf GitHub anzeigen",
      firstParagraph:
         "Diese umfassende Plattform aggregiert die Verfügbarkeit von Berghütten aus zwei Hauptquellen: Hütten, die mit dem zentralen Buchungssystem der Alpenvereine der Schweiz, Österreich, Deutschland und Südtirol (Italien) verbunden sind, sowie Unterkünfte von huetten-holiday.com.",
      secondParagraph:
         "Unser System aktualisiert kontinuierlich die Verfügbarkeitsinformationen, um Bergsteigern aktuelle Belegungsinformationen zu bieten. Benutzer können die Verfügbarkeit von Hunderten von Berghütten im gesamten Alpenraum suchen, filtern und verfolgen.",
      thirdParagraph:
         "Dieses Projekt wird als Open-Source entwickelt. Der vollständige Quellcode ist auf GitHub verfügbar.",
   },
   countries: {
      Schweiz: "Schweiz",
      Österreich: "Österreich",
      Deutschland: "Deutschland",
      Italien: "Italien",
      Slowenien: "Slowenien",
      Belgien: "Belgien",
      Frankreich: "Frankreich",
      unbekannt: "unbekannt",
   },
};
