            ...
            <div class="control-group">
               ...
               <div class="input-group">
                  <select v-model="selectedBedCategory" class="form-control">
                     ...
                     <option v-for="option in this.bedCategories" :key="option.name" :value="option.name">
                        {{ option.name }}
                     </option>
                  </select>
               </div>
      ...
      <div id="mainmap" style="z-index: 1">
         ...
         >
            ...
            <l-tile-layer
               ...
               layer-type="base"
            />
            <template v-for="hut in this.huts" :key="hut.id">
               <l-marker
                  ref="markerItems"
                  :name="hut.name"
               ...
               >
                  <l-tooltip>
                     <b>{{ hut.name }}</b>
                     <div v-if="hut.availability != null && !hut.availability.hutClosed">
                        {{ $t("mapPage.freeBeds") }}: {{ hut.availability.totalFreeBeds }} /
                        {{ hut.availability.totalBeds }}
                     </div>
                     <div v-if="hut.availability?.hutClosed">
                        <i>{{ $t("message.hutClosed") }}</i>
                     </div>
                     <!-- Blue marker -->
                     <div v-if="!hut.enabled">
                        <i>{{ $t("message.hutNotYetActive") }}</i>
                     </div>
                     <!-- Grey marker -->
                  </l-tooltip>
                  <l-popup
                     :options="{ closeButton: false, maxWidth: 320, minWidth: 320, className: 'custom-popup' }"
                     @remove="onPopupClose"
                  >
                     <div class="hut-popup-content">
                        <div class="hut-popup-header">
                           <h3 class="hut-popup-title">
                              <router-link
                                 :to="{ name: 'hutDetailsPage', params: { hutId: hut.id } }"
                        ...
                        <div class="hut-popup-body">
                           <div class="popup-status-row">
                              <div
                                 v-if="hut.availability != null && !hut.availability.hutClosed"
                                 class="availability-indicator"
                              >
                                 <div
                                    class="availability-badge"
                                    :class="
                                       getAvailabilityClass(hut.availability.totalFreeBeds, hut.availability.totalBeds)
                                    "
                                 >
                           ...
                           <div class="popup-action-buttons">
                              ...
                              <a
                                 v-if="hut.hutWebsite != null"
                                 :href="`${hut.hutWebsite}`"
                                 target="_blank"
                                 class="popup-btn btn-secondary"
                              >
                           ...
                           <div
                              v-if="hut.availability != null && hut.availability.roomAvailabilities?.length > 0"
                              class="room-availability-table"
                           >
                              <h4 class="section-title">{{ $t("message.typeOfAccommodation") }}</h4>
                              <table>
                                 <tbody>
                                    <tr
                                       v-for="availability in hut.availability?.roomAvailabilities"
                                       :key="availability.bedCategory"
                           ...
                           <div
                              v-if="!hut.availability?.hutClosed && hut.availability?.totalFreeBeds == 0"
...
export default {
   ...
   data() {
      return {
         ...
         desiredNumberOfBeds: (() => {
            ...
            if (
               queryNumBeds &&
               !isNaN(parseInt(queryNumBeds)) &&
               parseInt(queryNumBeds) > 0 &&
               parseInt(queryNumBeds) <= 10
            ) {
               return parseInt(queryNumBeds);
            }
            return 1;
         })(),
         mapCenter: [48.0, 11.33],
         zoom: (() => {
   ...
   watch: {
      dateFilter: async function (newValue) {
      ...
      desiredNumberOfBeds: function (newValue) {
         if (newValue < 1 || newValue > 10) {
            this.desiredNumberOfBeds = 1;
         }
         this.updateQueryParams("numBeds", newValue);
      },
      selectedBedCategory: function (newValue) {
